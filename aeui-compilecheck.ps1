# Compile-check helper for the AlterEyes UI package while the editor holds the lock.
# Usage: powershell -File aeui-compilecheck.ps1 -Target runtime|editor|tests|all
...
function Compile($outDll, $defines, $refs, $sources) {
    ...
    $tmpRsp = "$Tmp\aeui_csc.rsp"
    $a -join "`n" | Out-File -Encoding utf8 $tmpRsp
    & dotnet "$csc" "@$tmpRsp" 2>&1 | Where-Object { "$_" -match "error" } | ForEach-Object { Write-Host "$_" }
    return $LASTEXITCODE
}

$runtimeOut = "$Tmp\AEUI_Runtime.dll"
$editorOut  = "$Tmp\AEUI_Editor.dll"
$editoruiOut= "$Tmp\AEUI_EditorUI.dll"
...
if ($Target -eq "tests" -or $Target -eq "all") {
    ...
    if ((Compile "$Tmp\AEUI_TestsEdit.dll" @("UNITY_EDITOR","UNITY_INCLUDE_TESTS") $tRefs $tSrc) -ne 0) { Write-Host "EDIT TESTS FAILED" -ForegroundColor Red; exit 1 }
    ...
    if ((Compile "$Tmp\AEUI_TestsPlay.dll" @("UNITY_EDITOR","UNITY_INCLUDE_TESTS") $tRefs $pSrc) -ne 0) { Write-Host "PLAY TESTS FAILED" -ForegroundColor Red; exit 1 }
