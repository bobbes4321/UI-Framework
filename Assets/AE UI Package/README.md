# AlterEyes UI Package (`com.altereyes.ui`)

Doozy UI Manager 4 replacement, implementing the
[feature spec](../docs/altereyes-ui-package-feature-spec.md). Runtime assembly `AlterEyes.UI`,
editor assembly `AlterEyes.UI.Editor`, tests under `Tests/`.
...
| §12 Editor UX | `Editor/` | Editor heartbeat, edit-mode preview controls (Play/Reverse/From/To/Stop with clean state revert) on all animator inspectors, fast category/name pickers with inline add-new, one settings asset (`Resources/AEUISettings`) |
...
| Rendering | `Runtime/Graphics` | `AEShape` — SDF vector graphic (rounded rect with per-corner radius, circle, pill, checkmark/chevron/cross glyphs; border, linear/radial gradient, soft shadow via `edgeSoftness`), one shared material (`Resources/AEShape.shader`), params ride vertex channels so everything batches; `AEGradient` vertex-gradient mesh effect (theme-token stops, works on TMP) |
| Shape styles | `Runtime/Theming` | `ShapeStyle` (named radius/border/softness + token colors) on the `Theme`; `ThemeShapeStyleTarget` binds an `AEShape` to a style by name with live edit-mode restyle |
| Starter kit | `Editor/StarterKitBootstrap.cs` | `Tools → AlterEyes UI → Create or Repair Starter Kit`: full Dark/Light palette + shape styles on the theme, and a sprite-free prefab library (Button, Toggle, Switch, Slider, ProgressBar, Card, TabBar, ListView, Tooltip, Popup, Showcase view) under `Starter/` |

## Quick start

1. `Tools → AlterEyes UI → Create or Repair Settings` creates `Resources/AEUISettings.asset`,
...
4. Create a Flow Graph (`Create → AlterEyes → UI → Flow Graph`), open it (double-click), add a
   `FlowController` to a scene object. In play mode the window highlights the active node live.

## Agent workflow (spec → assets)

- **Generate:** `Tools → AlterEyes UI → Generate From Spec…` (or
...
- **Export:** `Tools → AlterEyes UI → Export Spec…` dumps theme/presets/views/flow back to JSON.
- **Validate:** `Tools → AlterEyes UI → Validate`, or in CI:
  `Unity -batchmode -projectPath <proj> -executeMethod AlterEyes.UI.Editor.AgentValidation.ValidateFromBatchMode`
  (exit code 0 = clean).
- **See your output:** `UIScreenshotter` renders any UI prefab to PNG without play mode —
  menu (`Screenshot Selected Prefab`), batch
  (`-executeMethod AlterEyes.UI.Editor.UIScreenshotter.CaptureFromCommandLine -aeuiPrefab <assetPath> -aeuiOut <png>`),
  or — while the editor is open — enable `Tools → AlterEyes UI → Screenshot Watcher` once and drop
  `{"prefab":"Assets/...","out":"shot.png","width":1080,"height":1920}` at
  `Temp/aeui-screenshot-request.json`; the result report appears at `Temp/aeui-screenshot-result.json`.
...
generates ready-made `UIPopup` prefabs. Widgets are built by `UIWidgetFactory` (AEShape-based,
