...
{
    /// <summary>
    /// "Tools → AlterEyes UI → Create or Repair Starter Kit": expands the default theme into a
    /// full design-system palette (Dark + Light variants, surface shape styles) and generates the
    /// reusable widget prefab library under <c>Assets/AE UI Package/Starter</c> — every prefab
    /// built from AEShape (zero sprites) through <see cref="UIWidgetFactory"/>. Idempotent like
    ...
    {
        public const string StarterFolder = "Assets/AE UI Package/Starter";
        public const string DarkVariant = "Dark";
        public const string LightVariant = "Light";

        [MenuItem("Tools/AlterEyes UI/Create or Repair Starter Kit", priority = 101)]
        public static void CreateOrRepairMenu()
        {
            GenerateReport report = CreateOrRepair();
            Debug.Log($"[AlterEyes.UI] Starter kit:\n{report}");
        }

        public static GenerateReport CreateOrRepair()
        {
            var report = new GenerateReport();
            AEUISettings settings = AEUISettingsBootstrap.EnsureSettings();
        ...
        private static GameObject BuildShowcase(AEUISettings settings)
        {
        ...
        private static GameObject SavePrefab(GenerateReport report, AEUISettings settings,
