...
{
    /// <summary>
    /// "Tools → AlterEyes UI → Create or Repair Fonts": generates the committed TMP SDF font
    /// assets for the package's Inter family (Regular/SemiBold/Bold) next to their TTFs under
    /// <c>Assets/AE UI Package/Fonts</c>. One-time and idempotent — existing assets are reused,
    ...
    {
        public const string FontsFolder = "Assets/AE UI Package/Fonts";
        ...
        [MenuItem("Tools/AlterEyes UI/Create or Repair Fonts", priority = 102)]
        public static void CreateOrRepairMenu()
        {
            ...
            TMP_FontAsset icons = EnsureIconFont(AEUISettings.instance);
            Debug.Log($"[AlterEyes.UI] Fonts: regular={(regular != null)}, semiBold={(semiBold != null)}, " +
        ...
        /// </summary>
        public static TMP_FontAsset EnsureIconFont(AEUISettings settings)
        {
        ...
        public static TMP_FontAsset EnsureFontAsset(string ttfPath, string assetPath, string characters = null)
        {
            ...
            if (font == null)
            {
                Debug.LogWarning($"[AlterEyes.UI] Font file '{ttfPath}' not found — cannot create '{assetPath}'");
                return null;
            }
            ...
            if (fontAsset == null)
            {
                Debug.LogWarning($"[AlterEyes.UI] TMP could not create a font asset from '{ttfPath}'");
                return null;
            }
            ...
            if (!string.IsNullOrEmpty(missing))
                Debug.LogWarning($"[AlterEyes.UI] '{assetName}': glyphs missing from source font: {missing}");
