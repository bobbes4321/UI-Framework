...
namespace AlterEyes.UI.Editor
{
    ...
    public class FlowTriggerDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            EditorGUI.PropertyField(AEGUI.NextLine(ref line), typeProperty, label);

            var type = (FlowTrigger.TriggerType)Mathf.Max(0, typeProperty.enumValueIndex);
            switch (type)
            {
                case FlowTrigger.TriggerType.Timer:
                    EditorGUI.PropertyField(EditorGUI.IndentedRect(AEGUI.NextLine(ref line)),
                ...
                default:
                    Rect pair = EditorGUI.IndentedRect(AEGUI.NextLine(ref line));
    ...
    public class FlowEdgeDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (property.isExpanded)
            {
                ...
                EditorGUI.PropertyField(AEGUI.NextLine(ref line), portNameProperty);
                DrawTargetNodeDropdown(AEGUI.NextLine(ref line), toNodeProperty);
                EditorGUI.PropertyField(AEGUI.NextLine(ref line), property.FindPropertyRelative("allowsBack"));
                ...
                if (OwnerIsRandomNode(property))
                    EditorGUI.PropertyField(AEGUI.NextLine(ref line), property.FindPropertyRelative("weight"));
                EditorGUI.indentLevel--;
            }
            EditorGUI.EndProperty();
        }
        ...
        private static void DrawTargetNodeDropdown(Rect rect, SerializedProperty toNodeProperty)
        {
            ...
            AEDropdown.ValuePopup(rect, current, () => TargetNodeOptions(graph), selected =>
            {
