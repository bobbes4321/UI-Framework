...
namespace AlterEyes.UI.Editor
{
    ...
    {
        [MenuItem("Tools/AlterEyes UI/Export Spec…", priority = 2)]
        public static void ExportToFileDialog()
        {
            ...
            Debug.Log($"[AlterEyes.UI] Spec exported to {path}");
        }
        ...
        private static string FindMatchingPresetName(AEUISettings settings, UIAnimation animation)
        {
        ...
        private static ElementSpec ExportElementBody(GameObject go, bool inLayout)
        {
            ...
            if (scroll != null)
            {
                ...
                if (go.GetComponent<AEShape>() == null) element.background = "none";
            ...
            var aeShape = go.GetComponent<AEShape>();
            if (aeShape != null)
            {
        ...
        {
            var gradient = go.GetComponent<AEGradient>();
        ...
        /// <summary>
        /// Card decor on a container's layout host (the generator adds an AEShape when a stack/grid/
        ...
        {
            var shape = go.GetComponent<AEShape>();
