...
namespace AlterEyes.UI.Editor
{
    ...
    /// - menu "Tools → AlterEyes UI → Screenshot Selected Prefab"
    /// - batch: <c>-executeMethod ... UIScreenshotter.CaptureFromCommandLine -aeuiPrefab &lt;assetPath&gt;
    ///   -aeuiOut &lt;png&gt; [-aeuiWidth 1080 -aeuiHeight 1920]</c>
    /// - with the editor open: the <see cref="AgentBridge"/> "screenshot" action.
    /// </summary>
    public static class UIScreenshotter
    {
        public const string DefaultOutputFolder = "Temp/aeui-screenshots";
        ...
        /// <summary> Renders the prefab to a PNG (default: Temp/aeui-screenshots/&lt;name&gt;.png). </summary>
            ...
            Func<Scene, RectTransform, GameObject> produceContent)
        {
            ...
            try
            {
                var cameraGo = new GameObject("AEUI Screenshot Camera");
                ...
                var canvasGo = new GameObject("AEUI Screenshot Canvas", typeof(RectTransform));
        ...
        [MenuItem("Tools/AlterEyes UI/Screenshot Selected Prefab", priority = 3)]
        public static void CaptureSelected()
        {
            foreach (GameObject prefab in Selection.gameObjects)
            {
                string path = Capture(prefab);
                Debug.Log($"[AlterEyes.UI] Screenshot → {Path.GetFullPath(path)}", prefab);
            }
        }

        [MenuItem("Tools/AlterEyes UI/Screenshot Selected Prefab", validate = true)]
        private static bool CaptureSelectedValidate() => Selection.gameObjects.Length > 0;

        // ------------------------------------------------------------------ batch mode

        /// <summary> -executeMethod entry: reads -aeuiPrefab/-aeuiOut/-aeuiWidth/-aeuiHeight. </summary>
        public static void CaptureFromCommandLine()
        {
            ...
            for (int i = 0; i < args.Length - 1; i++)
            {
                switch (args[i])
                {
                    case "-aeuiPrefab": prefabPath = args[i + 1]; break;
                    case "-aeuiOut": outPath = args[i + 1]; break;
                    case "-aeuiWidth": int.TryParse(args[i + 1], out width); break;
                    case "-aeuiHeight": int.TryParse(args[i + 1], out height); break;
                }
            }
            if (string.IsNullOrEmpty(prefabPath))
                throw new ArgumentException("Missing -aeuiPrefab <assetPath>");
            string written = CapturePrefab(prefabPath, outPath, width, height);
            Debug.Log($"[AlterEyes.UI] Screenshot → {Path.GetFullPath(written)}");
        }
    }
}
