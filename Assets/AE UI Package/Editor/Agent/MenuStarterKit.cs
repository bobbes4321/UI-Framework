...
namespace AlterEyes.UI.Editor
{
    ...
    /// <c>AEUISettings.menuWidgets</c>. Author-time generation embeds the same factory widgets directly,
    /// so spec'd menus and runtime-built menus share one widget vocabulary.
    /// </summary>
    public static class MenuStarterKit
    {
        private const string Folder = "Assets/AE UI Package/Starter/Menus";

        [MenuItem("Tools/AlterEyes UI/Create or Repair Menu Widget Library", priority = 22)]
        public static void CreateOrRepair()
        {
            ...
            Debug.Log($"[AlterEyes.UI] Menu widget library ready at {Folder} and assigned to AEUISettings.");
        }
