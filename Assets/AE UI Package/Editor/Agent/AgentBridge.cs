...
namespace AlterEyes.UI.Editor
{
    ...
    /// "Tools → AlterEyes UI → Agent Bridge"). Drop JSON at <c>Temp/aeui-request.json</c>; the
    /// result lands at <c>Temp/aeui-result.json</c>. One request at a time, polled cheaply
    ...
    /// <c>Unity.exe -batchmode -projectPath . -executeMethod AlterEyes.UI.Editor.AgentBridge.RunBatch
    /// -aeuiRequest requests.json -aeuiResult results.json</c> — the request file holds one request
    /// object or an array (processed in order; omit -nographics when screenshots/previews are in it).
    /// </summary>
    public static class AgentBridge
    {
        public const string RequestPath = "Temp/aeui-request.json";
        public const string ResultPath = "Temp/aeui-result.json";
        private const string MenuPath = "Tools/AlterEyes UI/Agent Bridge";
        private const string PrefsKey = "AlterEyes.UI.AgentBridge";
        ...
        private static void Toggle()
        {
            ...
            Debug.Log($"[AlterEyes.UI] Agent bridge {(enable ? "enabled" : "disabled")} (watching {RequestPath})");
        }
        ...
        /// <c>-executeMethod AlterEyes.UI.Editor.AgentBridge.RunBatch -aeuiRequest req.json
        /// -aeuiResult res.json</c>
        /// </summary>
        public static void RunBatch()
        {
            string[] args = Environment.GetCommandLineArgs();
            string requestPath = ArgValue(args, "-aeuiRequest");
            string resultPath = ArgValue(args, "-aeuiResult") ?? "aeui-batch-result.json";
            int exitCode = 0;
            var results = new List<object>();
            try
            {
                if (string.IsNullOrEmpty(requestPath) || !File.Exists(requestPath))
                    throw new ArgumentException($"-aeuiRequest must point at an existing request JSON file (got '{requestPath}')");
