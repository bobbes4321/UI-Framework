...
namespace Neo.UI
{
    ...
    /// </summary>
    [AddComponentMenu("Neo/UI/Flow Controller")]
    public class FlowController : MonoBehaviour, ITickable
    {
        ...
        public void StartFlow()
        {
            if (flow == null)
            {
                Debug.LogWarning("[Neo.UI] FlowController has no flow graph assigned — nothing to start.", this);
                return;
            }
            ...
            else Debug.LogWarning($"[Neo.UI] FlowGraph '{flow.name}' has no start node.", this);
        }
        ...
        public bool SetActiveNodeByName(string nodeName)
        {
            ...
            {
                Debug.LogWarning($"[Neo.UI] FlowController has no executable node named '{nodeName}'.", this);
                return false;
            }
            SetActiveNode(node);
            return true;
