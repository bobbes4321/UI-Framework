...
namespace Neo.UI
{
    /// <summary> The geometric primitives <see cref="NeoShape"/> can render. </summary>
    public enum ShapeType
    {
        ...
        Chevron = 4,
        Cross = 5,
        /// <summary> Annulus hugging the rect's inscribed circle; see <see cref="NeoShape.ringThickness"/>. </summary>
        Ring = 6,
        /// <summary> Ring segment (rounded caps) for radial progress/dials; start/sweep in degrees clockwise from 12 o'clock. </summary>
        Arc = 7
    }

    /// <summary> How <see cref="NeoShape"/> corner radii are interpreted. </summary>
    ...
    /// <summary> How <see cref="NeoShape"/> fills its interior. </summary>
    ...
    /// <summary> How an <see cref="NeoShape"/> texture fill maps onto the rect. </summary>
    ...
    /// channels (UV1-UV3 + tangent) so every NeoShape shares ONE material and batches normally.
    /// The inherited <see cref="Graphic.color"/> is the fill color, so ThemeColorTarget, color
    /// animators and tweens drive an NeoShape exactly like an Image.
    /// </summary>
    [AddComponentMenu("Neo/UI/Rendering/Shape")]
    [RequireComponent(typeof(CanvasRenderer))]
    public class NeoShape : MaskableGraphic
    {
        public const string ShaderName = "Neo/UI/Shape";
        /// <summary> Resources name of the shader asset (fallback when Shader.Find misses). </summary>
        public const string ShaderResourcesPath = "NeoShape";
        ...
        /// <summary> The single material every NeoShape shares (masked variants derive from it). </summary>
        public static Material sharedShapeMaterial
        {
            get
            {
                ...
                if (shader == null)
                {
                    Debug.LogWarning($"[Neo.UI] Shader '{ShaderName}' not found; NeoShape falls back to UI/Default");
                    return defaultGraphicMaterial;
                }
                s_sharedMaterial = new Material(shader)
                {
                    name = "NeoShape (Shared)",
