...
namespace Neo.UI.Editor
{
    ...
    public class UIActionBehaviourDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (property.isExpanded)
            {
                ...
                EditorGUI.PropertyField(NeoGUI.NextLine(ref line), triggerProperty);
                ...
                EditorGUI.PropertyField(NeoGUI.NextLine(ref line), sendSignalProperty);
                if (sendSignalProperty.boolValue)
                    EditorGUI.PropertyField(NeoGUI.NextLine(ref line), property.FindPropertyRelative("signalStream"));

                EditorGUI.PropertyField(NeoGUI.NextLine(ref line), property.FindPropertyRelative("cooldown"));
                EditorGUI.indentLevel--;
            }
            EditorGUI.EndProperty();
        }
    ...
    public class TweenSettingsDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            {
                EditorGUI.indentLevel++;
                EditorGUI.PropertyField(NeoGUI.NextLine(ref line), property.FindPropertyRelative("playMode"));
                DrawEaseRow(NeoGUI.NextLine(ref line), property);
                DrawValueOrRangeRow(NeoGUI.NextLine(ref line), property, "Duration", "duration", "useRandomDuration", "randomDuration");
                DrawValueOrRangeRow(NeoGUI.NextLine(ref line), property, "Start Delay", "startDelay", "useRandomStartDelay", "randomStartDelay");
                DrawValueOrRangeRow(NeoGUI.NextLine(ref line), property, "Loops", "loops", "useRandomLoops", "randomLoops");
                if (HasLoops(property))
                    DrawValueOrRangeRow(NeoGUI.NextLine(ref line), property, "Loop Delay", "loopDelay", "useRandomLoopDelay", "randomLoopDelay");

                if (IsSpringOrShake(property))
                {
                    EditorGUI.PropertyField(NeoGUI.NextLine(ref line), property.FindPropertyRelative("strength"));
                    EditorGUI.PropertyField(NeoGUI.NextLine(ref line), property.FindPropertyRelative("vibration"));
                    EditorGUI.PropertyField(NeoGUI.NextLine(ref line), property.FindPropertyRelative("elasticity"));
                    EditorGUI.PropertyField(NeoGUI.NextLine(ref line), property.FindPropertyRelative("fadeOutShake"));
                }
                EditorGUI.indentLevel--;
            }
            EditorGUI.EndProperty();
            ...
            string label, string valueName, string toggleName, string rangeName)
        {
            ...
            if (toggleProperty.boolValue)
            {
                ...
                NeoGUI.SplitHorizontal(valueRect, out Rect minRect, out Rect maxRect);
