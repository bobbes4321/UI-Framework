# Neo UI Package — Onboarding Guide
...
- Exactly **one settings asset** (`Resources/NeoUISettings`).
...
```
Assets/Neo UI Framework/
├── Runtime/   → asmdef Neo.UI         (all gameplay-facing runtime code)
├── Editor/    → asmdef Neo.UI.Editor  (inspectors, drawers, flow window, agent tooling)
│   └── EditorUI/ → asmdef Neo.EditorUI (standalone editor-UI kit — zero deps on Neo.UI)
├── Tests/     → EditMode + PlayMode test asmdefs
├── Resources/ → NeoUISettings asset, ID databases, the NeoShape shader
...
- **`Neo.EditorUI` is liftable.** It is a standalone editor-tooling kit (NeoGUI, NeoColors,
  NeoStyles, NeoSearchablePopup, NeoDropdown, NeoListView). It must keep **zero references** to
  `Neo.UI` so it can be dropped into any other project. Don't add a dependency back.
...
- **`Runtime/Ids`** — `IdDatabase` ScriptableObjects per addressable kind (`ButtonIdDatabase`,
  ...
  databases live on `NeoUISettings`. `CategoryNameId` is the shared id struct.
...
### 4.8 Graphics (`Runtime/Graphics`) — the NeoShape system

**This is the visual foundation of the entire package.** `NeoShape` is an SDF (signed-distance-field)
vector graphic — rectangles, rounded rects, circles, rings, arcs — and **every visual is built from
it**. Its shader lives at `Resources/NeoShape.shader`. Shape parameters (corner radius, thickness, arc
sweep, gradients) ride **vertex channels (UV1–3 + tangent)**, so **one shared material batches
everything**.

> ⚠️ **Never give an NeoShape its own material.** Doing so breaks batching. Drive its look through its
> serialized shape parameters and the shared material.

`NeoGradient` (live theme-token gradients) and `ElevationRecipe` (shadow/depth presets) round out the
layer.
...
| An `NeoShape` surface (radius, border, fill, gradient, elevation) | `ThemeShapeStyleTarget` | `style` → a `ShapeStyle` on the theme (its `fillColor` is a token ref) |
...
The active theme is resolved lazily: `ThemeService.activeTheme` returns `NeoUISettings.theme` (the
single asset at `Resources/NeoUISettings`) unless overridden in code. A missing token resolves to
...
2. **Open the Theme asset** (the one in `NeoUISettings.theme`). Select the **active variant**, find
...
- **Whole-theme swap** — `ThemeService.activeTheme = otherTheme;` or assign a different asset to `NeoUISettings.theme`.
...
**Option A — curated bundle (fastest, recommended).** `Tools → Neo UI → Apply Theme Bundle`
...
1. **Create it:** Project window → `Create → Neo → UI → Theme` (or duplicate the starter theme
...
4. **Add text & shape styles (optional):** populate `textStyles` (`name`, `font`, `size`, style flags,
   ...
   by name.
5. **Make it active:** assign it to `NeoUISettings.theme` (run `Tools → Neo UI → Create or Repair
...
- **`NeoGUI`** — layout primitives (`BeginFoldoutSection`, headers, rows).
- **`NeoColors`** — category accent colors: Interactive=blue, Containers=cyan, Animation=orange,
  Flow=purple, Theming=pink, Signals=teal, Data=yellow.
- **`NeoStyles`** — cached `GUIStyle`s.
- **`NeoSearchablePopup`**, **`NeoDropdown`**, **`NeoListView`** — searchable/cached IMGUI controls.

### 5.2 Inspectors & drawers

- Inspectors subclass **`NeoUIEditor`** (`Editor/Inspectors/ComponentEditors.cs`). Selectable-based
...
The full field catalog is generated to **`Assets/docs/spec-reference.md`** + **`neo-spec.schema.json`**.

### 6.3 Driving the agent bridge (editor open)

Toggle **`Tools → Neo UI → Agent Bridge`** once, then write `Temp/neo-request.json` and read
`Temp/neo-result.json`. Actions:
...
- **Create or Repair Settings** — creates `Resources/NeoUISettings` + the ID databases (one settings asset max).
...
```
Unity.exe -executeMethod Neo.UI.Editor.BeautificationAcceptance.Run   # omit -nographics
```
...
2. Run **`Tools → Neo UI → Create or Repair Settings`**, then **Create or Repair Fonts**, then
   **Create or Repair Starter Kit**.
3. Optionally apply a theme bundle: **`Tools → Neo UI → Apply Theme Bundle`**.
